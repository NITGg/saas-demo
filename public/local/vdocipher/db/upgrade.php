...
/**
 * local_vdocipher upgrade steps.
 *
 * @param int $oldversion
 * @return bool
 */
function xmldb_local_vdocipher_upgrade($oldversion) {
    global $DB;
    $dbman = $DB->get_manager();

    // The mapping table was added to install.xml after the plugin's first
    // release, so existing installs need it created here (install.xml only runs
    // on a fresh install).
    if ($oldversion < 2026081202) {
        $table = new xmldb_table('local_vdocipher_videos');
        ...
        upgrade_plugin_savepoint(true, 2026081202, 'local', 'vdocipher');
    }
