...
/**
 * NIT learner dashboard layout (/my) — ROLE-AWARE.
 *
 * Learners get the T1 "Welcome back" dashboard rendered from their REAL enrolled
 * courses + completion progress (no design-only placeholders). Staff (course
 * creators / admins) and ANYONE in edit mode get Moodle's REAL block dashboard
 * (Boost drawers layout) instead, so blocks, edit mode, add-block and
 * reset-dashboard all keep working — we delegate to the stock layout rather than
 * reimplement it. This is the "skin for learners, real page for management" split.
 *
 * @package   theme_nit
 * @copyright 2026 NIT
...
// Staff or editing → the real, editable block dashboard (delegate to Boost).
